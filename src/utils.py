...
def select_subset(word_list, max_vocab, exclusion_list):
    """
    ...
    for i, word in enumerate(word_list):
        word = word.lower()
        if word not in word2id and word not in exclusion_list:
            word2id[word] = len(word2id)
            indexes.append(i)
...
def load_bin_embeddings(params, lang, emb_path, full_vocab):
    ...
    logger.info(f"Loaded binary model for {lang}. Generating embeddings ...")
    embeddings = torch.from_numpy(np.concatenate([model.get_word_vector(w)[None] for w in words], 0))
    logger.info(f"Generated embeddings for {len(words)} words in lang {lang}.")
    ...
    if not full_vocab:
        ...
        word2id, indexes = select_subset(words, params.max_vocab, words_to_exclude)
        embeddings = embeddings[indexes]
    else:
...
def load_fasttext_model_for_export(params, lang, emb_path):
    """
    Reloads the full fasttext model and also returns the input matrix to apply the mapping to.
    """
    assert emb_path.endswith('.bin')
    model = load_fasttext_model(emb_path)
    words = model.get_words()
    assert model.get_dimension() == params.emb_dim
    logger.info("Loaded binary model.")

    word2id = {w: i for i, w in enumerate(words)}
    id2word = {i: w for w, i in word2id.items()}
    dico = Dictionary(id2word, word2id, lang)

    return dico, torch.from_numpy(model.get_input_matrix()), model
...
def export_fasttext_embeddings(model, input_matrix: np.ndarray, lang, params):
    """
    Export fasttext embeddings by manually setting the input matrix and saving to a .bin file.
    """
    path = os.path.join(params.exp_path, 'vectors-%s.bin' % lang)
    logger.info('Writing embeddings to %s...' % path)
    model.set_matrices(input_matrix, model.get_output_matrix())
...
def apply_mapping_to_fasttext(mapping, input_matrix):
    bs = 4096
    with torch.no_grad():
        output_matrix = input_matrix.clone()
        for i, k in enumerate(range(0, len(input_matrix), bs)):
            x = input_matrix[k:k + bs]
            output_matrix[k:k + bs] = mapping(x.to(mapping.weight.device)).to(input_matrix.device)
