...
from .utils import export_fasttext_embeddings, get_optimizer, load_embeddings, normalize_embeddings, export_embeddings, load_fasttext_model_for_export
from .utils import clip_parameters, apply_mapping, apply_mapping_to_fasttext
...
class Trainer(object):
    ...
    def dis_step(self, stats):
        ...
        for lang2 in self.params.all_langs:
            ...
            i = 0
            while (not torch.all(torch.isfinite(x)) or (x != x).any()) and i < 100:
                x, y = self.get_dis_xy(lang1, lang2, volatile=True)
                i += 1

            preds = self.discriminators[lang2](x.detach())  # preds is a tensor of size 64 -> 2 concatenated batches of size 32:
                                                            # the first half with actual predictions and the second half with targets
            if (preds != preds).any():
                logger.error("Some elements are NaN!")
                logger.error(preds)
                logger.error(x)
                exit()
            if not torch.all(torch.isfinite(preds)):
                logger.error("Some elements are not finite!")
                logger.error(preds)
                logger.error(x)
                exit()
    ...
    def export(self):
        ...
        if params.export == "bin":
            dico, input_matrix, model = load_fasttext_model_for_export(params, params.tgt_lang,
                    params.tgt_emb)
            del dico
            del params.vocabs
            export_fasttext_embeddings(model, input_matrix.numpy(), params.tgt_lang, params)
        else:
            params.vocabs[params.tgt_lang], tgt_emb = load_embeddings(params, params.tgt_lang,
                    params.tgt_emb, full_vocab=True)
            export_embeddings(tgt_emb, params.tgt_lang, params)        
        # export all source embeddings
        for i, src_lang in enumerate(params.src_langs):
            logger.info(f"Map {src_lang} embeddings to the target space ...")
            if params.export == "bin":
                dico, input_matrix, model = load_fasttext_model_for_export(params, src_lang,
                        params.src_embs[i])
                del dico
                input_matrix = apply_mapping_to_fasttext(self.mappings[src_lang], input_matrix)
                del self.mappings[src_lang]
                export_fasttext_embeddings(model, input_matrix.numpy(), src_lang, params)
            else:
                ...
                export_embeddings(src_emb, params.src_lang, params)
